  ...
  <div>
    ...
    <Calling :items="calledList" :dialog="showCalling" :done-context="doneContext">
      <template v-slot:actions>
        <template v-if="showClose">
          <v-btn color="blue darken-1" flat @click.native="callStreamer">{{ $t('message.replay') }}</v-btn>
          <v-btn style="background:#e74c3c !important;" flat @click.native="showCalling=false">{{ $t('message.close') }}</v-btn>
        </template>
        ...
        <v-tooltip bottom>
          <template slot="activator">
            <v-avatar size="36" color="#fff">
              <v-btn icon large>
                <v-icon color="primary red--after">people</v-icon>
              </v-btn>
            </v-avatar>
          </template>
          <span>{{ $t('message.online') + $t('message.people') }}</span>
        </v-tooltip>
      </v-badge>
      <v-card :max-height="getClientHeight">
        <v-card-text>
          <v-text-field
            append-icon="search"
        ...
        <v-list v-for="(user, uindex) in onlineList" :key="user.user_id" :class="user.new && 'bounceOut new-item' ">
          <v-list-tile avatar>
            <v-list-tile-avatar>
              <img :src="$getImageLocation(user.avatar, 'avatar')" alt="avatar">
            </v-list-tile-avatar>

            <v-list-tile-content>
              <v-list-tile-title><span style="font-size: 12px;">{{ `${user.user_name}${user.full_name && ' (' + user.full_name +') '}` }}</span></v-list-tile-title>
              <v-list-tile-sub-title>
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-icon :color="user.new ? '#fff' : 'primary'" dark v-on="on">{{ user.client_type === 'web' ? 'computer' : 'phone_iphone' }}</v-icon>
                  </template>
                  <span>{{ user.client_type === 'web' ? $t("message['PC']") : $t("message['Mobile']") }}</span>
                </v-tooltip>
    ...
    >
      <template v-slot:activator="{ on }">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" @click="peopleMenu=true" icon large>
      ...
      <v-card>
        ...
        <v-list v-for="(user, uindex) in onlineList" :key="user.user_id" :class="user.new && 'bounceOut new-item' ">
          <v-list-tile avatar>
            <v-list-tile-avatar>
              <img :src="$getImageLocation(user.avatar)" alt="avatar">
            </v-list-tile-avatar>
            ...
            <v-list-tile-content>
              ...
              <v-list-tile-sub-title>
                <v-tooltip bottom>
                  ...
                  <span>{{ user.client_type === 'web' ? 'PC端' : '移动端' }}</span>
                </v-tooltip>
                <v-tooltip bottom v-if="user.user_id == $user.id">
                  <template v-slot:activator="{ on }">
                    <v-icon :color="user.new ? '#fff' : 'primary'" dark v-on="on">sentiment_satisfied</v-icon>
...
export default {
  ...
  computed: {
    ...
    showClose() {
      const notEnter = this.calledList.find(item => {
        return item.state === 0
      })
      return !notEnter
    }
      ...
      .then(res => {
        if (res.data.code === 1) {
          ...
          _this.$store.commit('setRequest', {
            ...
            url: ``,
            type: 'error',
            msg: _this.$t("message['Failed to get online list']")
          })
        }
      })
    },

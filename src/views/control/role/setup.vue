  	...
  	<v-container fluid grid-list-xl py-0>
			<v-layout row wrap>
				...
				>
					...
					<div class="card-title-library">{{ `角色名称：${power.name}， 当前项目：${activeProject.name} `}}</div>
					<v-card-title>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "角色名称", value: "ruleName", sortable: true },
				{ text: "授权", value: "status", sortable: true }
      ],
      items: [],
			message: {
				show: false,
	...
	beforeMount() {
		this.searchClass = new this.$Search({
			type: 'get',
			path: `role/assign?iipsearch=true&id=${this.power.name}`,
			body: {}
		}, 5000)
  ...
  methods: {
      ...
      if (this.pagination.sortBy === column) {
		...
		getPageData() {
			...
			this.isLoad = true
			if(this.pagination.getFullPage){
        Axios.get(`role/assign?pid=${this.activeProject.pid}&id=${this.power.name}&sort=${(_this.pagination.descending ? '-' : '') + this.pagination.sortBy}&page=${this.pagination.pageFake}&per-page=-1`)
        .then(res => {
        ...
        Axios.get(`role/assign?pid=${this.activeProject.pid}&id=${this.power.name}&sort=${(_this.pagination.descending ? '-' : '') + this.pagination.sortBy}&page=${this.pagination.pageFake}&per-page=${this.pagination.rowsPerPage}`)
        .then(res => {
          let dat = res.data.data
					dat = _this.setResultId(dat)
          _this.items = dat.result
      ...
      return params
		...
		givePower(props) {
			// 授权/取消授权
			let _this = this
      Axios.post(`role/remove?id=${this.power.name}`,
				{
      ...
      .then(res => {
        if(res.data.code === 1){
					...
					_this.$store.commit('setRequest', {
						...
						url: `role/create`,
						type: 'error',
						msg: `角色${(_this.items[props.index].status) ? '授权' : '解除授权'}失败`
					})
        }
      })
			.catch(() => {
...
//this.searchClass.searchChange(this.search)
			...
			this.searchClass.option.path = `role/assign?iipsearch=true&id=${this.power.name}&pid=${this.activeProject.pid}`
		}
  }
}
</script>

					...
					>
						...
						<template slot="items" slot-scope="props">
				          ...
				          ></v-checkbox>
								...
								<td class="text-xs-center" v-html="searchClass.setWordColor(props.item.value)"></td>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "路由名称", value: "value", sortable: true },
	...
	beforeMount() {
		...
		this.searchClass = new this.$Search({
			type: 'get',
			path: `route/index?iipsearch=true`,
			body: {}
		}, 5000)
  ...
  methods: {
      ...
      if (this.pagination.sortBy === column) {
		...
		getPageData() {
			...
			this.isLoad = true
			if(this.pagination.getFullPage){
        Axios.get(`route/index?pid=${this.activeProject.pid}&sort=${(_this.pagination.descending ? '-' : '') + this.pagination.sortBy}&page=${this.pagination.pageFake}&per-page=-1`)
        .then(res => {
        ...
        Axios.get(`route/index?pid=${this.activeProject.pid}&sort=${(_this.pagination.descending ? '-' : '') + this.pagination.sortBy}&page=${this.pagination.pageFake}&per-page=${this.pagination.rowsPerPage}`)
        .then(res => {
          let dat = res.data.data
					dat = _this.setResultId(dat)
          _this.items = dat.result
      ...
      .then(res => {
        if(res.data.code === 1){
					_this.$store.commit('setRequest', {
						...
						url: `role/create`,
						type: 'success',
						msg: `路由${(props.item.status) ? '启用' : '禁用'}成功`
					})
        }else {
					...
					_this.$store.commit('setRequest', {
						...
						url: `role/create`,
						type: 'error',
						msg: `路由${(props.item.status) ? '启用' : '禁用'}失败`
					})
        }
      })
			.catch(() => {
      ...
      .then(res => {
		...
		librarySelected(item) {
			...
			this.searchClass.initOption()
			//this.searchClass.searchChange(this.search)
			this.getPageData()
			this.searchClass.option.path = `route/index?iipsearch=true&pid=${this.activeProject.pid}`
		}
  }
}
</script>
